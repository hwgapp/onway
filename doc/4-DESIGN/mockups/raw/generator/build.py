...
def cover():
    ...
    for fid, name, _, chain, chain2, succ, edge in FLOWS:
        ch = " → ".join(f'<a href="{c}.dc.html" style="font-family: {MONO}; font-size: 12px; text-decoration: none; color: {C["t1"]}">{c}</a>' for c in chain)
        ch2 = (f'<div style="{TY["cap"]}; color: {C["t2"]}; margin-top: 4px">Tài xế / phụ: ' + " → ".join(f'<a href="{c}.dc.html" style="font-family: {MONO}; text-decoration: none; color: {C["t1"]}">{c}</a>' for c in chain2) + "</div>") if chain2 else ""
    ...
    root = (f'<div style="width: {W}px; height: {H}px; box-sizing: border-box; padding: 64px; background: #FFFFFF; font-family: {FONT}; color: {C["t1"]}; display: flex; flex-direction: column; gap: 32px; border: 1px solid {C["b2"]}">'
            ...
            + row(appc, gap=16)
            + f'<div><div style="{TY["ovl"]}; color: {C["t3"]}; margin-bottom: 4px">Luồng chính (bấm mã frame để mở — chế độ Play)</div>{flows}</div>'
            + "</div>")
    return root, W, H
...
def main():
    ...
    for p, frames in by_page.items():
        ...
        for gi, (gname, fs) in enumerate(groups):
            ...
            for f in fs:
                ...
                b = {"x": x, "y": y, "w": w, "h": hh, "title": title, "page": PAGE[p]}
                if inter:
    ...
    canvas = {"v": 3, "createdOnFiles": {"v": 1, "at": now}, "title": "Onway UI Workflow", "launch": {"view": "canvas", "page": "overview"}, "pages": pages,
